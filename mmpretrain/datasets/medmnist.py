...
class MedMNIST(BaseDataset):
    ...
    def __init__(self, split: str, **kwargs):
        ...
        self.split = split
        super(MedMNIST, self).__init__(ann_file='',
                                       **kwargs)

    def load_data_list(self):
        """Load images and ground truth labels."""
        backend = get_file_backend(self.data_prefix['img_path'], enable_singleton=True)
        
        if dist.is_main_process() and not self._check_integrity():
            if not isinstance(backend, LocalBackend):
                raise RuntimeError(f"The dataset on {self.data_prefix['img_path']} is not integrated, "
        ...
        assert self._check_integrity(), \
            ...
            f"Please download the dataset manually through {self.info['url']}."

        npz_file = np.load(
            os.path.join(self.data_prefix['img_path'], '{}.npz'.format(self.flag)))

        if self.split == 'train':
            self.imgs = npz_file['train_images']
            self.gt_labels = npz_file['train_labels'].squeeze()
        elif self.split == 'val':
            self.imgs = npz_file['val_images']
            self.gt_labels = npz_file['val_labels'].squeeze()
        elif self.split == 'test':
            self.imgs = npz_file['test_images']
            self.gt_labels = npz_file['test_labels'].squeeze()
        else:
            raise ValueError
        ...
        for i, (img, gt_label) in enumerate(zip(self.imgs, self.gt_labels)):
            ...
            info = {'img': img, 'gt_label': gt_label, 'medmnist_idx': i}
    ...
    def __repr__(self):
        ...
        body.append(f'Root location: {self.data_prefix["img_path"]}')
    ...
    def _check_integrity(self):
        npz_file = os.path.join(self.data_prefix['img_path'], '{}.npz'.format(self.flag))
        md5 = self.info['MD5']
        return check_integrity(npz_file, md5)
